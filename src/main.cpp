...
Stock myStocks[4]; Stock myIndices[2]; String myNews[10];  
...
void loadLocalData() {
  ...
  for (int i = 0; i < 4; i++) { myStocks[i].name = doc["stocks"][i]["name"].as<String>(); myStocks[i].price = doc["stocks"][i]["price"].as<String>(); myStocks[i].change = doc["stocks"][i]["change"].as<String>(); }
...
void drawClock() {
  ...
  M5.Display.setFont(&fonts::FreeSansBold24pt7b); M5.Display.setTextColor(M5.Display.color565(40, 40, 80)); 
  char timeStr[10]; sprintf(timeStr, "%02d:%02d", t.hours, t.minutes); M5.Display.drawCenterString(timeStr, 160, 52);
  M5.Display.drawFastHLine(15, 115, 290, BLACK); M5.Display.drawFastHLine(15, 235, 290, BLACK);
  if (isSdFontLoaded && globalFontBuffer) M5.Display.loadFont(globalFontBuffer);
  else M5.Display.setFont(&fonts::efontKR_24);
  int totalSteps = 13; int step = infoIndex % totalSteps;
  if (step < 10) {
    M5.Display.setTextColor(M5.Display.color565(200, 100, 0)); M5.Display.setCursor(20, 122); M5.Display.printf("NEWS %d/10", step + 1); 
    M5.Display.setTextColor(BLACK); M5.Display.setTextWrap(true); M5.Display.clearClipRect(); M5.Display.setClipRect(20, 148, 280, 85); 
    M5.Display.setCursor(20, 148); M5.Display.print(myNews[step]); M5.Display.clearClipRect();
  } else if (step == 10) {
    M5.Display.setTextColor(M5.Display.color565(0, 120, 150)); M5.Display.setCursor(20, 120); M5.Display.print("MARKET INDEX");
    for (int i = 0; i < 2; i++) {
        int yPos = 155 + (i * 38); M5.Display.setTextColor(BLACK); M5.Display.setCursor(20, yPos); M5.Display.print(myIndices[i].name); 
    ...
    M5.Display.setTextColor(M5.Display.color565(0, 150, 0)); M5.Display.setCursor(20, 120); M5.Display.printf("STOCKS %d/2", step - 10); 
    int startIdx = (step == 11) ? 0 : 2;
    for (int i = 0; i < 2; i++) {
        int sIdx = startIdx + i; int yPos = 155 + (i * 38); M5.Display.setTextColor(BLACK); M5.Display.setCursor(20, yPos); M5.Display.print(myStocks[sIdx].name); 
        M5.Display.setCursor(120, yPos); M5.Display.print(myStocks[sIdx].price);
        if (myStocks[sIdx].change.indexOf("+") != -1) M5.Display.setTextColor(RED); else if (myStocks[sIdx].change.indexOf("-") != -1) M5.Display.setTextColor(BLUE); else M5.Display.setTextColor(BLACK);
...
void drawLumiFace(float blink) {
  ...
  for (int i = -1; i <= 1; i += 2) {
    int x = centerX + (i * eyeDistance) + (int)(lookX * 20); int y = centerY + (int)(lookY * 15);
...
void loop() {
  M5.update();
  if (millis() - lastWeatherUpdate > 600000 && WiFi.status() == WL_CONNECTED) { fetchWeather(); fetchStocks(); fetchNews(); lastWeatherUpdate = millis(); }
